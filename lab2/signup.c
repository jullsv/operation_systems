...
int setup_server_socket() {
    ...
    printf("SRV: Listening on port %d. FD %d\n", PORT, server_fd);
    return server_fd;
}
...
void register_signal_handler() {
    ...
    printf("SRV: SIGHUP handler registered.\n");
}
...
void block_signal(sigset_t *origMask) {
    ...
    printf("SRV: SIGHUP blocked. Ready to run pselect().\n");
}
...
void main_loop(int server_fd, const sigset_t *origSigMask) {
    ...
    while (1) {
        ...
        if (FD_ISSET(server_fd, &fds)) {
            ...
            printf("EVT: New connection on FD %d.\n", new_socket);

            if (accepted_client_fd == -1) {
                accepted_client_fd = new_socket;
                printf("SRV: Connection accepted.\n");
            } else {
                printf("SRV: Connection rejected. Closing FD %d.\n", new_socket);
                close(new_socket);
            }
        }
        ...
        if (accepted_client_fd != -1 && FD_ISSET(accepted_client_fd, &fds)) {
            ...
            if (valread > 0) {
                printf("EVT: Data received from FD %d. Bytes: %zd\n", accepted_client_fd, valread);
            } else {
                printf("SRV: Client disconnected. Closing FD %d.\n", accepted_client_fd);
